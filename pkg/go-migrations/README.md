...
```
    bash $GOPATH/tyre-match-backend.com/mdm/pkg/go-migrations/init.sh
    or if you use vendor folder
    bash vendor/tyre-match-backend.com/mdm/pkg/go-migrations/init.sh
```

Or you can copy sources from your dependencies path manually if you have trouble with command.
For example from:
```
    .../tyre-match-backend.com/mdm/pkg/go-migrations/template
```
