...
// GetAllMigrations will return any migration that hasn't already been run.
func GetAllMigrations(connection *sqlx.DB) (bool, []string) {
	var raw []string
	err := connection.Select(&raw, getAllRawSQL())
	if err != nil {
		return false, raw
	}
	return true, raw
...
func getAllRawSQL() string {
	switch os.Getenv("DB_DRIVER") {
	case "mysql":
		return "SELECT name FROM migrations ORDER BY created_at ASC;"
	case "postgres":
		return "SELECT name FROM migrations ORDER BY created_at ASC;"
	default:
		panic("Not supported DB driver: " + os.Getenv("DB_DRIVER"))
